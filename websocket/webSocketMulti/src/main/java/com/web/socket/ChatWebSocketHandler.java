  ...
  http://limkydev.tistory.com/64
    */
    ...
    public void afterConnectionEstablished(
    	...
    	logger.info(new Date()+ " : "+session.getId() + " 연결 됨");
    ...
    public void afterConnectionClosed(
    	...
    	logger.info(new Date()+ " : "+session.getId() + " 연결 종료됨");
    ...
    protected void handleTextMessage(
    	...
    	logger.info(new Date() + " : "+session.getId() + "로부터 메시지 수신: " + message.getPayload());
...
//        Iterator<WebSocketSession> iwss = users.values().iterator();
//        while(users.values().iterator().hasNext()) {
//        	WebSocketSession wss = iwss.next();
...
//        
        for (WebSocketSession s : users.values()) { //<-- .values() 로 session들만 가져옴
            ...
            logger.info(new Date() + " : "+s.getId() + "에 메시지 발송: " + message.getPayload());
