...
$(document).ready(function(){
	...
	function window_container(data){
		...
		var url = "container.do?stId="+data.stId+"&chgrNm="+encodeURIComponent(data.chgrNm)+"&cmdTp="+data.cmdTp+"&resDt="+dt[0]+"d"+dt[1] ;
		window.open(url ,"container", 
		       "toolbar=no, width=900, height=700, directories=no, status=no,    scrollorbars=no, resizable=no");
	}
	
	
	function window_chgr(){
		var url = 'searchChgr.do?search='+encodeURIComponent($('#chgrId').val());
		window.open(url ,"chargerSearch", 
		       "toolbar=no, width=500, height=400, directories=no, status=no,    scrollorbars=no, resizable=no");
	}
